...
forval k=1/22 {
   ...
   forval j=1/`nclus' {
      ...
      !echo -e "/genetics/bin/qctool -g `F'/chr`k'.gen.gz -og chr`k'_`lowr'_`uppr'.gen -incl-range `lowr'-`uppr' -omit-chromosome -excl-rsids exc`k'_`lowr'_`uppr'.txt -sort;gzip -f chr`k'_`lowr'_`uppr'.gen" >> `T'/Extract.sh
   }
   restore
}
cd `T'
