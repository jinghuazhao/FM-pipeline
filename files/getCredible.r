...
bed <- read.table("st.bed", as.is=TRUE, col.names=c("chr","start","end",
                 "pos","rsid","r"), skip=as.numeric(args[1])+1, nrows=1)
f <- Sys.getenv("f")
dat <- read.table(paste0(f,".r"), as.is=TRUE, col.names=c("chr","pos","A","B","freqA",
                 "Effect","StdErr","P","TOTALSAMPLESIZE","SNP",'SNPID',"z"))
dat <- within(dat, P=2*pnorm(abs(z),lower.tail=FALSE))
...
write.table(subset(result,!is.na(inCredible)&!is.na(prob_norm)&!is.na(prob_cumsum)), 
            file=paste0(f,".cre"), sep="\t", quote=F, col.names=TRUE, row.names=FALSE)
